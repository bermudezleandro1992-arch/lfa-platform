...
export default function CeoPage() {
  ...
  useEffect(() => {
    ...
    subs.push(onSnapshot(doc(db,'configuracion','spawner'), d => {
      if (d.exists()) {
        const data = d.data() as SpawnerConfig & { simultaneous_per_tier?: SimCfg };
        setSpawnerCfg(data);
        if (data.simultaneous_per_tier) setSimultaneasCfg({ ...SIM_DEFAULT, ...data.simultaneous_per_tier });
      }
    }));
      ...
      for (const region of MEGA_REGIONS) {
        for (const capacity of MEGA_SIZES) {
          for (const { tier, pool } of MEGA_TIERS) {
            const tierKey = tier as keyof SimCfg;
            docsToCreate.push({
              ...
              activo: true,
              auto_respawn: true,
              max_simultaneous: simultaneasCfg[tierKey] ?? 1,
              created_at: serverTimestamp(),
            });
          }
        }
    ...
    <>
      ...
      <div style={{ margin:0, fontFamily:"'Roboto',sans-serif", background:'#0b0e14', color:'white', minHeight:'100vh' }}>
        ...
        <header style={{ background:'rgba(7,9,13,0.97)', display:'flex', alignItems:'stretch', borderBottom:'2px solid #ffd700', position:'sticky', top:0, zIndex:100, flexWrap:'wrap' }}>
          ...
          <a href="/soporte" style={{ display:'flex', alignItems:'center', padding:'0 16px', color:'#00ff88', textDecoration:'none', fontFamily:"'Orbitron',sans-serif", fontSize:'0.68rem', borderLeft:'1px solid #30363d', gap:6, whiteSpace:'nowrap' }}>🎫 SOPORTE</a>
                      ...
                      <tr key={j.id} className="crow" style={{ opacity: j.baneado ? 0.65 : 1 }}>
                        ...
                        <td style={{ ...td, minWidth:160 }}>
                          <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
                            ...
                            <button className="cact" style={sm(j.rol==='organizador' ? 'rgba(0,210,255,0.2)' : 'rgba(0,210,255,0.07)', j.rol==='organizador' ? '#00d2ff' : '#555')} onClick={async () => { const newRol = j.rol==='organizador' ? '' : 'organizador'; const ok = await alerta(newRol ? 'DAR ROL ORGANIZADOR' : 'QUITAR ORGANIZADOR', `¿${newRol ? 'Asignar' : 'Quitar'} rol organizador a ${j.nombre||j.id}?`, newRol ? 'info' : 'error'); if(ok) { await updateDoc(doc(db,'usuarios',j.id),{ rol: newRol }); } }} title={j.rol==='organizador' ? 'Quitar Organizador' : 'Hacer Organizador'}>🎙️{j.rol==='organizador' ? ' ✓' : ''}</button>
                            <button className="cact" style={sm(j.rol==='mod' ? 'rgba(110,64,201,0.3)' : 'rgba(110,64,201,0.07)', j.rol==='mod' ? '#a371f7' : '#555')} onClick={async () => { const newRol = j.rol==='mod' ? 'jugador' : 'mod'; const ok = await alerta(j.rol==='mod' ? 'QUITAR MODERADOR' : 'DAR ROL MODERADOR', `¿${j.rol==='mod' ? 'Quitar' : 'Asignar'} rol de Moderador a ${j.nombre||j.id}?\n\nEste rol da acceso al Panel de Moderadores (/moderador).\nNO otorga control de LFA Coins.`, j.rol==='mod' ? 'error' : 'info'); if(ok) { await updateDoc(doc(db,'usuarios',j.id),{ rol: newRol }); await alerta('LISTO', j.rol==='mod' ? `${j.nombre||j.id} ya no es moderador.` : `${j.nombre||j.id} es ahora MODERADOR 🛡️`, j.rol==='mod' ? 'info' : 'exito'); } }} title={j.rol==='mod' ? 'Quitar Moderador' : 'Hacer Moderador'}>🛡️{j.rol==='mod' ? ' ✓' : ''}</button>
