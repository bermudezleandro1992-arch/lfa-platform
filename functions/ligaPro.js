/* eslint-disable */
// ligaPro.js ÔÇö Liga 1vs1 PRO Cloud Functions (UTF-8)
...
exports.notifyLeagueMatchChange = onDocumentWritten({
    ...
    const buildMsg = (status, match) => {
        ...
        if (status === 'challenged') {
            return `ÔÜí ┬íTe desafiaron en SomosLFA PRO! ${league}. Jornada ${match.round}. Rival: ${match.player1_team || match.player2_team}. Ingres├í en somosifa.com/pro para coordinar.`;
        }
        if (status === 'validating') {
            return `­ƒöì Tu rival subi├│ el resultado. ${league}. Ten├®s 10 minutos para confirmar o disputar en somosifa.com/pro`;
        }
        if (status === 'closed') {
            return `Ô£à Partido cerrado. ${league}. Resultado registrado en somosifa.com/pro`;
        }
        if (status === 'dispute') {
            return `­ƒÜ¿ Disputa abierta. ${league}. Staff revisar├í y resolver├í en 24hs.`;
        }
        return null;
    };
